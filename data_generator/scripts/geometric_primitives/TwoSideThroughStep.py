...
class TwoSideThroughStep:
    def __init__(self):
        ...
        self.width_A = np.random.uniform(1, 4)
        self.width_B = np.random.uniform(self.width_A + 0.5, 4.5)
        self.depth = np.random.uniform(1, 9)
        self.middle_point = 5
        self.negative_start_point = -0.0001
        self.positive_start_point = 10.0001

        self.max_volume = 765.0153  # value determined by printing the volume of 3D object with max dimensions
        self.max_manufacturing_time = 1.45
    ...
    def manufacturing_time_calculation(self, _two_side_through_step):
        manufacturing_time = self.max_manufacturing_time * (_two_side_through_step.volume() / self.max_volume)
        if self.dir in ["direction_2", "direction_4", "direction_6", "direction_8"]:
    ...
    def transformation(self):
        ...
        _manufacturing_time = round(self.manufacturing_time_calculation(_two_side_through_step), 4)

        return _two_side_through_step, _manufacturing_time

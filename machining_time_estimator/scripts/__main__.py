...
from network_models.GraphConv import GraphConv
...
_parser.add_argument(
    "--application_mode",
    dest="application_mode",
    default="test",
    type=str,
    help='The application modes has "training" and "graph". When set to trained the framework uses the'
    ...
    ".FeaStConv.html#torch_geometric.nn.conv.FeaStConv, When set to graph, the GraphConvModel is"
...
_parser.add_argument(
    ...
    # default=DataImporter(os.getenv("TRAINING_DATASET_SOURCE"), os.getenv("TRAINING_DATASET_DESTINATION")).shuffle(),
    default=None,
...
if __name__ == "__main__":
    ...
    if _config.application_mode == "training":
        ManufacturingTimeRegression(_config).training()
    elif _config.application_mode == "test":
        rest_service_instance = RestService(ManufacturingTimeRegression(_config))
        rest_service_instance.app.run(host="0.0.0.0", port=5003, debug=False)

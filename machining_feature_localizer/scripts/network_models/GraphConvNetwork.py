...
class GraphConvNetwork(torch.nn.Module):
    def __init__(self, dataset, device, hyper_parameter):
        ...
        self.conv1 = GraphConv(self.dataset.num_features, self.hidden_channels)
        ...
        if self.number_conv_layers > 3:
            self.conv4 = GraphConv(self.hidden_channels, self.hidden_channels)
        if self.number_conv_layers > 4:
            self.conv5 = GraphConv(self.hidden_channels, self.hidden_channels)
        if self.number_conv_layers > 5:
            self.conv6 = GraphConv(self.hidden_channels, self.hidden_channels)
